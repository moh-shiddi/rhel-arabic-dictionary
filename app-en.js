...
class ArabicText {
  static stopWords = new Set([
    "aryd", "aby", "abghy", "ahtaj", "mmkn", "can", "kyf", "tryqh",
    "ly", "ldy", "andy", "from", "in", "aly", "to", "an", "ma", "hw",
    "hy", "this", "this", "dhlk", "or", "then", "with", "all", "bwasth",
    "alrja", "lw", "fdla", "aml", "shy", "shy"
  ].map(word => ArabicText.normalize(word)));

  static synonyms = new Map(Object.entries({
    "thmyl": ["Install", "Download", "install"],
    "Download": ["Install", "thmyl", "install"],
    "tnsyb": ["Install", "install"],
    "application": ["program", "hzmh", "package"],
    "program": ["application", "hzmh", "package"],
    "Delete": ["azalh", "algha", "remove", "delete"],
    "msh": ["Delete", "azalh", "remove"],
    "Start": ["bd", "start", "enable"],
    "ayqaf": ["wqf", "stop", "disable"],
    "rystart": ["aaadh", "Start", "restart"],
    "khdmh": ["syrfs", "service", "systemctl"],
    "syrfs": ["khdmh", "service", "systemctl"],
    "lwq": ["log", "logs", "journal", "log"],
    "lwqat": ["logs", "journal", "log"],
    "akhta": ["fshl", "mshakl", "error", "failed"],
    "mshklh": ["khta", "fshl", "troubleshooting", "diagnose"],
    "layaml": ["fshl", "stopped", "failed"],
    "hard": ["disk", "storage", "disk"],
    "msahh": ["hjm", "storage", "disk"],
    "ram": ["dhakrh", "memory"],
    "bwrt": ["Port", "port"],
    "fayrwwl": ["firewall", "firewall"],
    "user": ["account", "user"],
    "qrwb": ["mjmwah", "group"],
    "directory": ["evidence", "directory", "folder"],
    "bhth": ["find", "grep", "search"],
    "shbkh": ["connection", "network", "ip"],
    "antrnt": ["shbkh", "connection", "ping"],
    "permissions": ["adhwnat", "permission", "chmod", "chown"],
    "Compress": ["arshyf", "tar", "zip"],
    "Extract": ["Extract", "extract", "unzip"],
    "syrfr": ["server", "server", "host"],
    "dkhwl": ["connection", "ssh", "login"],
    "explanation": ["Concept", "mahw", "why", "concept"],
    "learning": ["path", "dwrh", "learning", "path"],
    "command": ["command", "syntax", "options"]
  }).map(([key, values]) => [
...
class KnowledgeEngineApp {
  ...
  applyTheme() {
    ...
    this.e.themeButton.title = dark ? "mode alnhary" : "Dark mode";
    this.e.themeButton.setAttribute("aria-label", dark ? "Enable mode alnhary" : "Enable dark mode");
  ...
  renderSuggestions() {
    ...
    this.suggestions.forEach((item, index) => {
      ...
      if (item.kind === "intent") {
        ...
        const targetTitle = analysis.target?.title_ar || "Show results recommended";
        button.innerHTML = ` <span><strong>${ArabicText.escape(analysis.intent.icon || "⌁")} I understood that you want: ${ArabicText.escape(analysis.intent.title_ar)}</strong><small>${ArabicText.escape(targetTitle)}</small></span> <span class="suggestion-type">nya dhkya</span> `;
      } else {
  ...
  renderResults() {
    ...
    this.e.resultsEyebrow.textContent = query ? (this.searchAnalysis?.intent ? "results mrtba by alnya" : "results Search almwhd") : "Knowledge graph";
    this.e.resultsTitle.textContent = query ? `results: ${query}` : viewTitle;
    const intentSummary = this.searchAnalysis?.intent ? ` — understood alnya: ${this.searchAnalysis.intent.title_ar}` : "";
    this.e.resultsSummary.textContent = `Done Find ${all.length} result from asl ${this.entities.length}${intentSummary}`;
    ...
    if (!this.e.loadMoreButton.hidden) this.e.loadMoreButton.textContent = `Show almzyd (${all.length - visible.length} mtbqya)`;
  }
  ...
  quickActionLabel(entity) {
    if (["task", "troubleshooting"].includes(entity.entity_type)) return "abda Path";
    if (entity.entity_type === "command") return "Copy alsygha";
    return "";
  }
  ...
  quickAction(entity) {
    ...
    else if (entity.entity_type === "command") this.copy(entity.syntax || entity.command_name, "Done Copy alsygha");
  }

  renderActiveFilters() {
    const chips = [];
    if (this.e.searchInput.value.trim()) chips.push({ key: "search", label: `bhth: ${this.e.searchInput.value.trim()}` });
  ...
  toggleFavorite(id) {
    if (this.favorites.has(id)) {
      ...
      this.showToast("Donet alRemove from favorites");
    } else {
      this.favorites.add(id);
      this.showToast("Donet alAdd to favorites");
    }
  ...
  configureDialogActions(entity) {
    ...
    this.e.dialogPrimaryCopyButton.textContent = entity.entity_type === "command" ? "Copy alexamples" : "Copy all Commands";
  }

  updateDialogFavoriteButton() {
    if (!this.currentEntity) return;
    const favorite = this.favorites.has(this.currentEntity.id);
    this.e.dialogFavoriteButton.textContent = favorite ? "★ in Favorites" : "☆ Favorites";
  }
  ...
  renderEntityHeader(entity) {
    ...
    const meta = [
      ...
      ["alisdarat", `RHEL ${(entity.supported_versions || []).join(" / ")}`],
      ["status", entity.status === "verified" ? "mwthq" : entity.status === "reviewed" ? "mraja" : "mswda"]
    ];
    if (entity.risk) meta.splice(3, 0, ["alRisk", this.riskLabels[entity.risk] || entity.risk]);
    return `
  ...
  renderTask(task) {
    ...
    const workflowLabel = workflowSession
      ? (workflowSession.state === "completed" ? "re Path" : "astkmal Path")
      : "abda Path almwjh";
    return ` <section class="guided-workflow-entry"> <div> <span class="eyebrow">Guided Workflow</span> <h3>nfdh this task through mode mwjh</h3> <p>yraja almttlbat, Displays step one in all mra, wyhfz Verification wFinal report.</p> ${workflowSession? `<div class="guided-workflow-entry__progress"><span>Saved progress</span><div><i style="width:${workflowProgress}%"></i></div><b>${workflowProgress}%</b></div>` : ""}
        </div>
        <button type="button" data-start-guided-workflow>${workflowLabel}</button>
      </section>
      ${task.goal_ar ? `<section class="dialog-section"><div class="callout"><strong>Goal:</strong> ${ArabicText.escape(task.goal_ar)}</div></section>` : ""}
  ...
  renderPrerequisites(task) {
    ...
    return `<section class="dialog-section"><div class="dialog-section__heading"><h3>Before you start</h3></div><ul class="bullet-list">${task.prerequisites_ar.map(item => `<li>${ArabicText.escape(item)}</li>`).join("")}</ul></section>`;
  }

  renderVariables(task) {
    if (!(task.variables || []).length) return "";
    return ` <section class="dialog-section"> <div class="dialog-section__heading"><h3>Task inputs</h3><span class="section-note">ttghyr Commands automatically when idkhal alqym</span></div> <div class="variables-grid">${task.variables.map(variable => `
  ...
  renderTaskProgress(task) {
    ...
    return `<section class="dialog-section"><div class="progress-panel"><div class="progress-panel__header"><span>tqdm Execution</span><span id="taskProgressText">${completeCount} from ${required.length} — ${percent}%</span></div><div class="progress-track"><div id="taskProgressBar" class="progress-bar" style="width:${percent}%"></div></div></div></section>`;
  }

  renderSteps(task) {
    const completed = this.getTaskProgress(task.id);
    return ` <section class="dialog-section"> <div class="dialog-section__heading"><h3>steps Execution</h3><span class="section-note">${task.steps?.length || 0} steps</span></div> <div class="steps-list">${(task.steps || []).map((step, index) => { const resolved = this.resolveCommand(step.command); return `<article class="step-card ${completed.has(step.id) ? "is-complete" : ""}" data-step-card="${ArabicText.escape(step.id)}">
            <input class="step-check" type="checkbox" data-task-step="${ArabicText.escape(step.id)}" ${completed.has(step.id) ? "checked" : ""} aria-label="Complete step">
  ...
  renderErrors(task) {
    ...
    return `<section class="dialog-section"><div class="dialog-section__heading"><h3>Common errors</h3></div><div class="error-list">${task.common_errors.map(error => `<article class="error-item"><h4>${ArabicText.escape(error.symptom_ar)}</h4><p><strong>Likely causes:</strong></p><ul>${(error.likely_causes_ar || []).map(item => `<li>${ArabicText.escape(item)}</li>`).join("")}</ul>${(error.checks || []).map((check, index) => `<div class="error-check"><strong>${ArabicText.escape(check.title_ar)}</strong><code>${ArabicText.escape(this.resolveCommand(check.command))}</code><small>${ArabicText.escape(check.expected_result_ar)}</small><button class="text-button" type="button" data-copy-error-check="${ArabicText.escape(check.command)}">Copy alCheck</button></div>`).join("")}${(error.fixes_ar || []).length ? `<p><strong>Recommended fixes:</strong></p><ul>${error.fixes_ar.map(item => `<li>${ArabicText.escape(item)}</li>`).join("")}</ul>` : ""}</article>`).join("")}</div></section>`;
  }
  ...
  renderSafety(task) {
    ...
    return `<section class="dialog-section"><div class="dialog-section__heading"><h3>Safety warnings</h3></div><div class="callout callout--danger"><ul class="rollback-list">${task.safety_notes_ar.map(item => `<li>${ArabicText.escape(item)}</li>`).join("")}</ul></div></section>`;
  }

  renderCommandReference(entity) {
    return ` <section class="dialog-section"><div class="dialog-section__heading"><h3>alsygha alaama</h3></div><code class="syntax-box">${ArabicText.escape(entity.syntax || entity.command_name)}</code>${entity.provided_by? `<p class="section-note">Provided by package: <code>${ArabicText.escape(entity.provided_by)}</code></p>`: ""}</section> <section class="dialog-section"><div class="dialog-section__heading"><h3>What does it do?</h3></div><div class="callout">${ArabicText.escape(entity.purpose_ar || entity.summary_ar)}</div></section> ${(entity.options || []).length? `<section class="dialog-section"><div class="dialog-section__heading"><h3>Important options and actions</h3></div><div class="option-list">${entity.options.map(item => `<article class="option-item"><code>${ArabicText.escape(item.option)}</code><div><strong>${ArabicText.escape(item.description_ar)}</strong>${item.example ? `<p><code>${ArabicText.escape(item.example)}</code></p>` : ""}</div></article>`).join("")}</div></section>` : ""}
  ...
  renderConcept(entity) {
    return ` <section class="dialog-section"><div class="dialog-section__heading"><h3>altaryf</h3></div><div class="callout">${ArabicText.escape(entity.definition_ar || entity.summary_ar)}</div></section> <section class="dialog-section"><div class="dialog-section__heading"><h3>lmadha yhmk?</h3></div><p>${ArabicText.escape(entity.why_it_matters_ar || "")}</p></section> ${(entity.mental_model_ar || "")? `<section class="dialog-section"><div class="dialog-section__heading"><h3>Mental model</h3></div><div class="mental-model">${ArabicText.escape(entity.mental_model_ar)}</div></section>` : ""}
  ...
  renderLearningPath(entity) {
    ...
    return ` <section class="dialog-section"><div class="callout"><strong>wsf Path:</strong> ${ArabicText.escape(entity.description_ar || entity.summary_ar)}</div></section> <section class="dialog-section"><div class="progress-panel"><div class="progress-panel__header"><span>tqdm path learning</span><span id="pathProgressText">${completedCount} from ${modules.length} — ${percent}%</span></div><div class="progress-track"><div id="pathProgressBar" class="progress-bar" style="width:${percent}%"></div></div></div></section> <section class="dialog-section"><div class="dialog-section__heading"><h3>modules Path</h3><span class="section-note">almda altqdyrya ${entity.estimated_hours || 0} hours</span></div><div class="module-list">${modules.map((item, index) => { const target = this.entityById.get(item.entity_id); const done = completed.has(item.entity_id); return `<article class="module-item ${done ? "is-complete" : ""}" data-module-card="${ArabicText.escape(item.entity_id)}"><span class="module-number">${index + 1}</span><div><h4>${ArabicText.escape(target?.title_ar || item.entity_id)} ${item.optional ? '<span class="meta-badge">Optional</span>' : ""}</h4><p>${ArabicText.escape(item.objective_ar)}</p></div><div class="module-actions"><label><input type="checkbox" data-path-module="${ArabicText.escape(item.entity_id)}" ${done ? "checked" : ""}> Done</label><button type="button" data-open-entity="${ArabicText.escape(item.entity_id)}" ${target ? "" : "disabled"}>Open</button></div></article>`;
      }).join("")}</div></section>
      ${(entity.outcomes_ar || []).length ? `<section class="dialog-section"><div class="dialog-section__heading"><h3>What will you master?</h3></div><ul class="bullet-list">${entity.outcomes_ar.map(item => `<li>${ArabicText.escape(item)}</li>`).join("")}</ul></section>` : ""}
    `;
  }
  ...
  renderKnowledgeGraph(entity) {
    ...
    return ` <section class="dialog-section"> <div class="dialog-section__heading"><h3>khryta alIdentify</h3><span class="section-note">aCompress on any aqda llantqal</span></div> <div class="graph-wrap"><svg class="knowledge-graph" viewBox="0 0 ${width} ${height}" role="img" aria-label="Relationships for ${ArabicText.escape(entity.title_ar)}">${lines}${relatedNodes}${centerNode}</svg></div> <div class="related-list">${related.map(item => `<button class="related-button" type="button" data-open-entity="${ArabicText.escape(item.id)}">${ArabicText.escape(this.typeLabels[item.entity_type])}: ${ArabicText.escape(item.title_ar)}</button>`).join("")}</div>
      </section>
    `;
  }
  ...
  handleDialogClick(event) {
    ...
    if (copyStep) {
      ...
      if (step) this.copy(this.resolveCommand(step.command), "Done Copy Command");
      return;
    }
    ...
    if (copyVerification) {
      ...
      if (item) this.copy(this.resolveCommand(item.command), "Done Copy verification command");
      return;
    }
    const errorCheck = event.target.closest("[data-copy-error-check]");
    if (errorCheck) {
      this.copy(this.resolveCommand(errorCheck.dataset.copyErrorCheck), "Done Copy amr alCheck");
      return;
    }
    ...
    if (commandExample) {
      ...
      if (item) this.copy(item.command, "Done Copy example");
    }
  }
  ...
  renderIntentPanel() {
    ...
    const confidenceLabels = { high: "confidence High", medium: "confidence Medium", low: "Suggestion mhDonel" };
    ...
    this.e.intentPanelContent.innerHTML = ` <div class="intent-panel__inner"> <div class="intent-panel__header"> <div class="intent-panel__identity"> <span class="intent-panel__icon" aria-hidden="true">${ArabicText.escape(analysis.intent.icon || "⌁")}</span> <div><span class="eyebrow">Natural language analysis</span><h2>I understood that you want: ${ArabicText.escape(analysis.intent.title_ar)}</h2><p>${ArabicText.escape(analysis.intent.summary_ar || "")}</p></div> </div> <span class="intent-confidence intent-confidence--${analysis.confidence}">${confidenceLabels[analysis.confidence] || "Suggestion"}</span> </div> ${corrections? `<div class="correction-note"><strong>Technical correction:</strong> ${ArabicText.escape(corrections)}</div>` : ""}
        ${extracted ? `<div class="intent-analysis-row">${extracted}</div>` : ""}
        <div class="intent-target">
          <div class="intent-target__top"><div><h3>${target ? `Recommended workflow: ${ArabicText.escape(target.title_ar)}`: "results Search recommended"}</h3><p>${target? ArabicText.escape(target.summary_ar): "Results were ranked by the detected intent."}</p></div></div> ${fields? `<div class="intent-fields">${fields}</div>` : ""}
          <div class="intent-actions">
            ${target ? `<button class="intent-primary" type="button" data-intent-open>Open workflow and prepare commands</button>`: ""} <button class="intent-secondary" type="button" data-intent-scroll>Show results almrtba</button> <button class="intent-secondary" type="button" data-intent-dismiss>astkhdam Search alaady</button> </div> </div> ${alternatives.length? `<div class="intent-alternatives"><span>You may also mean:</span>${alternatives.map(item => `<button class="intent-alt" type="button" data-intent-alt="${ArabicText.escape(item.intent.id)}">${ArabicText.escape(item.intent.title_ar)}</button>`).join("")}</div>` : ""}
        ${analysis.reasons?.length ? `<div class="intent-reasons">Selection reason: ${ArabicText.escape(analysis.reasons.join(", "))}</div>` : ""}
      </div>`;
  ...
  updateTaskProgressUI(task) {
    ...
    if (text) text.textContent = `${count} from ${required.length} — ${percent}%`;
  ...
  updatePathProgressUI(path) {
    ...
    if (text) text.textContent = `${count} from ${modules.length} — ${percent}%`;
    if (bar) bar.style.width = `${percent}%`;
  }
  ...
  copyPrimaryContent() {
    ...
    if (["task", "troubleshooting"].includes(this.currentEntity.entity_type)) {
      ...
      this.copy(text, "Done Copy jmya awamr task");
    } else if (this.currentEntity.entity_type === "command") {
      const text = (this.currentEntity.examples || []).map(item => `# ${item.title_ar}\n${item.command}`).join("\n\n");
      this.copy(text || this.currentEntity.syntax, "Done Copy examples Command");
    }
  }
