...
class GuidedWorkflowRunner {
  constructor(app) {
    ...
    this.filter = "all";
    this.stageMeta = {
      prepare: ["1", "Preparation", "Requirements and inputs"],
      execute: ["2", "Execution", "Commands step by step"],
      verify: ["3", "Verification", "Confirm the result"],
      issues: ["4", "Diagnostics", "Analysis findings and checks"],
  ...
  pause() {
    ...
    this.save();
    this.close();
    this.app.showToast("The workflow was saved and can be resumed later");
  }
  ...
  renderLibrary() {
    ...
    this.e.workflowSidebar.innerHTML = ` <span class="wf-side-label">aljlsat</span> ${[["all","all Workflows"],["active","In progress"],["completed","Completed"]].map(([v,l]) => `<button data-wf-filter="${v}" class="${this.filter===v?"active":""}"><span>${l}</span><b>${
    ...
    `;
    this.e.workflowContent.innerHTML = ` <section class="wf-library"> <header><span class="eyebrow">Guided Workflow Library</span><h2>akhtr mhma lbd session mnzma</h2> <p>aladaa tShow Commands wthfz altqdm, lknha not tnfdh shyya on jhazk.</p></header> <div class="wf-library-controls"> <input id="wfSearch" type="search" value="${ArabicText.escape(this.query)}" placeholder="Search for a task or problem..."> <div>${[["all","All"],["active","In progress"],["completed","Completed"],["task","Tasks"],["troubleshooting","Troubleshooting"]].map(([v,l]) => `<button data-wf-filter="${v}" class="${this.filter===v?"active":""}">${l}</button>`).join("")}</div>
        </div>
  ...
  libraryCard(task) {
    ...
    return `<article class="wf-card"> <header><div><span class="type-badge type-${task.entity_type}">${task.entity_type==="troubleshooting"?"⚕ Troubleshooting":"✓ mhma"}</span> <span class="category-badge">${ArabicText.escape(this.app.data.categories[task.category]||task.category)}</span></div> ${s?`<b class="wf-state ${s.state}">${this.stateLabel(s.state)}</b>`:""}</header> <h3>${ArabicText.escape(task.title_ar)}</h3><p>${ArabicText.escape(task.summary_ar)}</p> <div class="wf-meta"><span>${task.steps.length} steps</span><span>${task.estimated_minutes||0} minutes</span><span>${this.app.difficultyLabels[task.difficulty]||task.difficulty}</span></div> ${s?`<div class="wf-mini"><span>Progress ${progress}%</span><i><b style="width:${progress}%"></b></i></div>`:""} <footer><button data-wf-start="${task.id}">${s&&s.state!=="completed"?"astkmal":"bd Path"}</button> <button data-wf-details="${task.id}">page alIdentify</button> ${s?`<button class="delete" data-wf-delete="${task.id}" title="Delete session">×</button>`:""}</footer>
    </article>`;
  }
    ...
    return `<div class="wf-progress"><strong>${p}%</strong><span>${this.stateLabel(this.session.state)}</span></div>
      <nav class="wf-stages">${stages.map(stage=>{
        ...
        return `<button data-wf-stage="${stage}" class="${current?"current":""} ${done?"done":""}">
          <span>${done?"✓":icon}</span><div><strong>${label}</strong><small>${desc}</small></div></button>`; }).join("")}</nav> <div class="wf-side-meta"><span>almda <b>${this.task.estimated_minutes||0}d</b></span><span>alsteps <b>${this.task.steps.length}</b></span></div>`;
  }
  ...
  prepare() {
    ...
    return `<section class="wf-stage">${this.heading("almrhla 1","jhz task before execution","raja almttlbat wadkhl alqym that ststbdl variables inside Commands.")} <div class="wf-goal"><strong>${ArabicText.escape(t.goal_ar||t.title_ar)}</strong><p>${ArabicText.escape(t.summary_ar||"")}</p></div> ${pre.length?`<div class="wf-panel"><h3>Preparation checklist</h3><div class="wf-checklist">${pre.map((x,i)=>`<label class="${s.prerequisites[i]?"checked":""}"><input type="checkbox" data-wf-pre="${i}" ${s.prerequisites[i]?"checked":""}><span>✓</span><strong>${ArabicText.escape(x)}</strong></label>`).join("")}</div></div>`:""}
      ${vars.length?`<div class="wf-panel"><h3>Task inputs</h3><p>Commands update automatically when values are entered.</p><div class="wf-vars">${vars.map(v=>`<label><span>${ArabicText.escape(v.label_ar)} ${v.required?"<b>Required</b>":""}</span><input data-wf-var="${v.name}" value="${ArabicText.escape(s.variables[v.name]||"")}" placeholder="Example: ${ArabicText.escape(v.example)}"><code>&lt;${v.name}&gt;</code></label>`).join("")}</div></div>`:""} <div class="wf-warning"><strong>tnbyh</strong><p>level alRisk: ${ArabicText.escape(this.app.riskLabels[t.risk]||t.risk||"Low")}. raja all amr before tnfydhh.</p></div> </section>`;
  }
  ...
  execute() {
    ...
    return `<section class="wf-stage">${this.heading(`Phase 2 — ${index+1}/${steps.length}`,step.title_ar,step.explanation_ar)}
      <div class="wf-dots">${steps.map((x,i)=>`<button data-wf-step="${i}" class="${i===index?"current":""} ${s.stepStatus[x.id]||"pending"}">${s.stepStatus[x.id]==="success"?"✓":s.stepStatus[x.id]==="issue"?"!":s.stepStatus[x.id]==="skipped"?"↷":i+1}</button>`).join("")}</div> <article class="wf-command ${status}"> <header><div><b>${index+1}</b><div><h3>${ArabicText.escape(step.title_ar)}</h3><p>${ArabicText.escape(step.explanation_ar)}</p></div></div> <div>${step.requires_sudo?"<span>sudo</span>":""}${step.optional?"<span>Optional</span>":""}<span>${this.app.riskLabels[step.risk]||step.risk}</span></div></header> <div class="wf-code"><code>${ArabicText.escape(command)}</code><button data-wf-copy-step>Copy</button></div> ${unresolved.length?`<div class="wf-unresolved">Complete these variables: ${unresolved.map(x=>`<code>&lt;${x}&gt;</code>`).join(" ")} <button data-wf-prepare>Edit inputs</button></div>`:""}
        ${step.expected_result_ar?`<div class="wf-expected"><strong>Expected result</strong><p>${ArabicText.escape(step.expected_result_ar)}</p></div>`:""} <section class="wf-output-analyzer"> <header><div><span>⌁</span><div><strong>Paste command output</strong><small>Analysis runs locally in your browser</small></div></div><b>No data is sent</b></header> <textarea data-wf-step-output="${ArabicText.escape(step.id)}" spellcheck="false" placeholder="Paste the complete terminal output here...">${ArabicText.escape(record.output||"")}</textarea> <div class="wf-analyzer-actions"> <button class="analyze" data-wf-analyze-step ${unresolved.length?"disabled":""}>Analyze output</button> <button data-wf-silent-step ${unresolved.length?"disabled":""}>Command succeeded with no output</button> <button data-wf-clear-analysis>Clear analysis</button> ${step.optional?`<button data-wf-outcome="skipped">Skip step</button>`:""}
          </div>
        </section>
  ...
  verify() {
    ...
    if(!item) return `<section class="wf-stage">${this.heading("almrhla 3","not twjd checks thqq","can alantqal to Final report.")}</section>`;
    const status=this.session.verificationStatus[i]||"pending";
    const record=this.diagnosticRecord("verify",String(i));
    return `<section class="wf-stage">${this.heading(`Phase 3 — ${i+1}/${list.length}`,"Verify task success","nfdh amr Verification walsq ntyjth lyhdd system alnjah or cause alfshl.")} <div class="wf-verify-tabs">${list.map((x,n)=>`<button data-wf-verify="${n}" class="${n===i?"current":""} ${this.session.verificationStatus[n]||"pending"}"><b>${this.session.verificationStatus[n]==="success"?"✓":this.session.verificationStatus[n]==="failed"?"!":n+1}</b>${ArabicText.escape(x.title_ar)}</button>`).join("")}</div> <article class="wf-command ${status}"><h3>${ArabicText.escape(item.title_ar)}</h3><div class="wf-code"><code>${ArabicText.escape(this.resolve(item.command))}</code><button data-wf-copy-verify>Copy</button></div> <div class="wf-expected"><strong>expected</strong><p>${ArabicText.escape(item.expected_result_ar||"result twkd njah process.")}</p></div> <section class="wf-output-analyzer"> <header><div><span>⌁</span><div><strong>Paste verification output</strong><small>The system will determine whether the result is successful or reveals a problem</small></div></div><b>Local analysis</b></header> <textarea data-wf-verify-output="${i}" spellcheck="false" placeholder="Paste verification command output...">${ArabicText.escape(record.output||"")}</textarea> <div class="wf-analyzer-actions"><button class="analyze" data-wf-analyze-verify>Analyze verification</button><button data-wf-clear-analysis>Clear analysis</button></div> </section> ${this.diagnosticHtml(record,"verify",String(i))} </article> </section>`;
  }

  issues() {
    const errors=this.task.common_errors||[],findings=this.allDiagnosticFindings();
    return `<section class="wf-stage">${this.heading("almrhla 4","Diagnostic center","Displays results that astntjha almhrk, Add to problems almsjla in qaCounta alIdentify.")} ${findings.length?`<div class="wf-collected-findings"><h3>Findings detected during this session</h3>${findings.map(f=>`<article class="${f.result.status}"><header><span>${f.result.status==="success"?"✓":f.result.status==="issue"?"!":"?"}</span><div><strong>${ArabicText.escape(f.result.title_ar)}</strong><small>${ArabicText.escape(f.source)} — confidence ${f.result.confidence||0}%</small></div></header><p>${ArabicText.escape(f.result.explanation_ar||"")}</p>${(f.result.evidence||[]).length?`<pre>${ArabicText.escape(f.result.evidence.join("\\n"))}</pre>`:""}</article>`).join("")}</div>`:""}
      ${errors.length?`<div class="wf-errors"><h3>Task-specific troubleshooting knowledge</h3>${errors.map((e,ei)=>`<details ${ei===0&&!findings.length?"open":""}><summary><b>!</b><strong>${ArabicText.escape(e.symptom_ar)}</strong></summary><div> <h4>Likely causes</h4><ul>${(e.likely_causes_ar||[]).map(x=>`<li>${ArabicText.escape(x)}</li>`).join("")}</ul>
      ...
      </div></details>`).join("")}</div>`:`<div class="wf-empty">No task-specific errors are registered, but the general analyzer remains available in every step.</div>`} <div class="wf-inline"><button data-wf-return>Return to the failed step</button>${(this.task.rollback_ar||[]).length?`<button data-wf-rollback>Open rollback</button>`:""}<button data-wf-doctor>Open Linux Doctor</button></div> </section>`;
  }
  rollback() {
    const list=this.task.rollback_ar||[];
    return `<section class="wf-stage">${this.heading("almrhla 5","Rollback wReturn to the previous state","raja athr all bnd before tnfydhh.")} <div class="wf-danger"><strong>zone hsasa</strong><p>qd ttdmn Delete package or Stop service. anshy Copya ahtyatya when alhaja.</p></div> ${list.length?`<div class="wf-rollback">${list.map((x,i)=>`<article class="${this.session.rollbackStatus[i]==="done"?"done":""}"><input type="checkbox" data-wf-rollback-item="${i}" ${this.session.rollbackStatus[i]==="done"?"checked":""}><div><strong>bnd ${i+1}</strong>${this.commandLike(this.resolve(x))?`<div class="wf-code"><code>${ArabicText.escape(this.resolve(x))}</code><button data-wf-copy-rollback="${i}">Copy</button></div>`:`<p>${ArabicText.escape(this.resolve(x))}</p>`}</div></article>`).join("")}</div>`:`<div class="wf-empty">No rollback plan is registered</div>`}
    </section>`;
  }
  ...
  completeView() {
    ...
    return `<section class="wf-stage">${this.heading("almrhla latest",s.state==="completed"?"Path mkDonel":"Summary and report","raja result wahfz altqryr.")} <div class="wf-final ${ready?"ready":"gaps"}"><b>${ready?"✓":"!"}</b><div><strong>${ready?"jmya alsteps alRequireda najha":"twjd anasr thtaj mrajaa"}</strong><p>progress current ${this.progress(t,s)}%</p></div></div> <div class="wf-summary"><article><span>alsteps</span><strong>${ok}/${required.length}</strong></article><article><span>التحقق alnajh</span><strong>${Object.values(s.verificationStatus).filter(x=>x==="success").length}</strong></article><article><span>fshl Verification</span><strong>${failed}</strong></article><article><span>status</span><strong>${this.stateLabel(s.state)}</strong></article></div> <pre class="wf-report">${ArabicText.escape(this.report())}</pre> <div class="wf-final-actions">${s.state!=="completed"?`<button class="complete" data-wf-complete>${ready?"inha kmkDonel":"inha with mlahzat"}</button>`:""}<button data-wf-copy-report>Copy report</button><button data-wf-download>Download TXT</button><button data-wf-restart>session new</button></div> </section>`;
  }

  footer(stages) {
    const i=stages.indexOf(this.session.stage);
    this.e.workflowPreviousButton.disabled=i<=0;
    this.e.workflowPreviousButton.textContent=i>0?`alsabq: ${this.stageMeta[stages[i-1]][1]}`:"alsabq";
    this.e.workflowNextButton.hidden=i>=stages.length-1;
    if(i<stages.length-1)this.e.workflowNextButton.textContent=`altaly: ${this.stageMeta[stages[i+1]][1]}`;
  ...
  next() {
    ...
    if(this.session.stage==="prepare"){
      ...
      if(missing.length){this.app.showToast(`akml alhqwl: ${missing.join(", ")}`);return;}
    }
  ...
  onClick(e) {
    ...
    const del=e.target.closest("[data-wf-delete]");if(del&&confirm("Delete session almhfwza?")){delete this.sessions[del.dataset.wfDelete];SafeStorage.set("rhel-ke:workflow-sessions",this.sessions);this.updateDashboard();this.renderLibrary();return;}
    ...
    if(e.target.closest("[data-wf-copy-step]")){const x=this.task.steps[this.session.stepIndex];this.app.copy(this.resolve(x.command),"Done Copy amr alstep");return;}
    if(e.target.closest("[data-wf-copy-verify]")){const x=this.task.verification[this.session.verificationIndex];this.app.copy(this.resolve(x.command),"Done Copy verification command");return;}
    ...
    if(e.target.closest("[data-wf-copy-followup]")){const {scope,key}=this.currentDiagnosticScope(),r=this.diagnosticRecord(scope,key);if(r.activeAction)this.app.copy(this.resolve(r.activeAction.command),"Done Copy amr alCheck");return;}
    ...
    const er=e.target.closest("[data-wf-copy-error]");if(er){const [a,b]=er.dataset.wfCopyError.split(":").map(Number),x=this.task.common_errors[a].checks[b];this.app.copy(this.resolve(x.command),"Done Copy amr alCheck");return;}
    const rb=e.target.closest("[data-wf-copy-rollback]");if(rb){this.app.copy(this.resolve(this.task.rollback_ar[Number(rb.dataset.wfCopyRollback)]),"Done Copy bnd Rollback");return;}
    ...
    if(e.target.closest("[data-wf-copy-report]")){this.app.copy(this.report(),"Done Copy report");return;}
    if(e.target.closest("[data-wf-download]")){this.download();return;}
    if(e.target.closest("[data-wf-restart]")&&confirm("msh progress wStart a new session?")){this.restart();}
  }
  ...
  currentDiagnosticCommand(scope) {
    if(scope==="verify"){
      ...
      return {command:this.resolve(item?.command||""),title:item?.title_ar||"Check Verification"};
    }
    const step=this.task.steps[this.session.stepIndex];
    return {command:this.resolve(step?.command||""),title:step?.title_ar||"step Execution"};
  }

  analyzeCurrent(scope) {
    if(!this.app.executionDiagnosticEngine)return this.app.showToast("mhrk althlyl not mtah");
    ...
    if(!output)return this.app.showToast("Paste command output awla, or astkhdm option alnjah bla output");
  ...
  silentStepSuccess() {
    ...
    const result={status:"success",confidence:70,id:"silent-manual-success",title_ar:"Done takyd njah amr bla output",
      explanation_ar:"user akd an Command antha without rsala error. ybqa Check Verification allahq hw manual alaqwa.",
      evidence:["not ytba Command any output"],likely_causes_ar:[],checks:[],fixes_ar:[],
  ...
  analyzeFollowup() {
    ...
    if(!output)return this.app.showToast("alsq result alCheck altaly");
    const result=this.app.executionDiagnosticEngine.analyze({
  ...
  diagnosticHtml(record,scope,key) {
    if(!record.history.length)return `<div class="wf-analysis-placeholder"><span>⌁</span><div><strong>Waiting for terminal output</strong><p>After analysis, the failed stage, likely cause, next check, and remediation will appear here.</p></div></div>`;
    ...
    return `<section class="wf-diagnosis ${status}"> <header><span>${icon}</span><div><small>${statusLabel}</small><h3>${ArabicText.escape(result.title_ar||"Analysis result")}</h3></div><b>confidence ${result.confidence||0}%</b></header> <p class="wf-diagnosis-explanation">${ArabicText.escape(result.explanation_ar||"")}</p> ${(result.evidence||[]).length?`<div class="wf-evidence"><strong>Evidence from output</strong><pre>${ArabicText.escape(result.evidence.join("\n"))}</pre></div>`:""}
      ...
      ${verification.length?`<div class="wf-next-checks verification"><h4>Verify remediation</h4>${verification.map((x,i)=>this.diagnosticActionCard(x,"verify",i)).join("")}</div>`:""}
      ...
      <div class="wf-diagnosis-actions">
        ${status==="success"?(scope==="step"?`<button class="accept" data-wf-accept-step>Accept success and continue</button>`:`<button class="accept" data-wf-accept-verify>Accept verification and continue</button>`):""}
  ...
  diagnosticActionCard(item,type,index) {
    ...
    return `<article><div><strong>${ArabicText.escape(item.title_ar||"Check")}</strong><small>${ArabicText.escape(item.expected_result_ar||"")}</small></div><div class="wf-code"><code>${ArabicText.escape(this.resolve(item.command||""))}</code><button ${attr}>Use this check</button></div></article>`;
  }

  followupHtml(record) {
    const action=record.activeAction;
    return `<section class="wf-followup"><header><span>→</span><div><strong>${ArabicText.escape(action.title_ar)}</strong><small>Run the check and paste its output to continue diagnosis</small></div></header> <div class="wf-code"><code>${ArabicText.escape(this.resolve(action.command||""))}</code><button data-wf-copy-followup>Copy</button></div> ${action.expected_result_ar?`<p>Expected: ${ArabicText.escape(action.expected_result_ar)}</p>`:""} <textarea data-wf-followup-output spellcheck="false" placeholder="Paste check output here...">${ArabicText.escape(record.followupOutput||"")}</textarea> <button class="analyze" data-wf-analyze-followup>Analyze check output</button> </section>`;
  }
  ...
  allDiagnosticFindings() {
    ...
    for(const [key,record] of Object.entries(this.session.verificationDiagnostics||{})){
      const title=this.task.verification?.[Number(key)]?.title_ar||`thqq ${Number(key)+1}`;
  ...
  stateLabel(s){return({active:"In progress",paused:"mhfwza",completed:"mkDonela"})[s]||"not tbda";}
  statusLabel(s){return({pending:"not tbda",success:"najha",issue:"twjd problem",skipped:"Done altkhty",failed:"fshl"})[s]||s;}
  ...
  report() {
    const t=this.task,s=this.session,lines=["tqryr path tnfydh RHEL","================================",`task: ${t.title_ar}`,`status: ${this.stateLabel(s.state)}`,`progress: ${this.progress(t,s)}%`,`bdat: ${this.date(s.startedAt)}`,"","steps Execution","----------------"];
    t.steps.forEach((x,i)=>{
      ...
      if(history.length){const last=history.at(-1).result;lines.push(` Diagnostics: ${last.title_ar} — confidence ${last.confidence||0}%`);if(last.evidence?.length)lines.push(` manual: ${last.evidence.join(" | ")}`);}
    ...
    if(s.issueNotes)lines.push("","mlahzat problem",s.issueNotes);
    lines.push("","tnbyh: this session talymya wla tany an Commands nfdht automatically.");
    return lines.join("\n");
  }
  ...
  download() {
    ...
    this.app.showToast("Done Download report");
  }
